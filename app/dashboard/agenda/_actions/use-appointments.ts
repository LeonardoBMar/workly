...
export function useAppointments(options: UseAppointmentsOptions = {}) {
  ...
  const transformAppointmentToBooking = (
    ...
    async (id: string, bookingData: Partial<Booking>) => {
      ...
      try {
        ...
        if (bookingData.start)
          updatePayload.startTime = bookingData.start.toISOString();
        if (bookingData.end)
          updatePayload.endTime = bookingData.end.toISOString();
        if (bookingData.serviceId)
          updatePayload.serviceId = bookingData.serviceId;
        if (bookingData.notes !== undefined)
          updatePayload.notes = bookingData.notes;
