...
export default function Header() {
  ...
  return (
    <header className="glass fixed top-0 z-50 w-full border-b border-white/10">
        ...
        <div className="flex items-center gap-4">
          {isPending ? (
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <Link
                href="/dashboard"
                className="flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 transition-all hover:bg-slate-200 sm:px-4 sm:py-2"
              >
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full text-slate-500 transition-all hover:bg-slate-100"
                title="Sair"
              >
                <LogOut className="h-4 w-4" />
              </button>
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-600 text-xs font-bold text-white uppercase shadow-lg ring-2 shadow-indigo-200 ring-white">
      ...
      {isMenuOpen && (
        <div className="animate-in slide-in-from-top-2 absolute top-16 left-0 w-full border-b border-slate-100 bg-white p-4 duration-200 md:hidden">
          <nav className="flex flex-col gap-4">
            {navItems.map((item) => (
              <Link
                ...
                className="px-2 py-1 text-base font-medium text-slate-700 hover:text-indigo-600"
              >
                {item.label}
              </Link>
            ))}
